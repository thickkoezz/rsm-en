...
// Implementation for parsing an Item into a RuntimeDef
impl RuntimeDef {
	// Try to parse a syntax Item into a RuntimeDef
	pub fn try_from(item: syn::Item) -> syn::Result<Self> {
		...
		let item_struct = if let syn::Item::Struct(item) = item {
			// Item is a struct, use it
			item
		} else {
			// Item is not a struct, return an error
		...
		// Get the name of the Runtime struct
		...
		for field in item_struct.fields.into_iter().skip(1) {
			// Each field should have an identifier (field name)
			if let Some(ident) = field.ident {
				// Store the field name and field type as a tuple
				pallets.push((ident, field.ty))
			}
		}

		// Return the parsed RuntimeDef
...
fn check_system(item_struct: &syn::ItemStruct) -> syn::Result<()> {
	...
	if first_field_name != "system" {
		// First field is not named system - error!
		let msg = "first field is expected to be named system";
		return Err(syn::Error::new(item_struct.span(), msg))
	}

	// Return success - system pallet is correctly placed
	Ok(())
}
